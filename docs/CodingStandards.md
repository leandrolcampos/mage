...
- [Languages, Libraries, and Standards](#languages-libraries-and-standards)
...
- [Mechanical Source Issues](#mechanical-source-issues)
  ...
  - [Language and Compiler Issues](#language-and-compiler-issues)
    - [Treat Compiler Warnings Like Errors](#treat-compiler-warnings-like-errors)
    - [Write Portable Code](#write-portable-code)
    - [Do Not Use RTTI or Exceptions](#do-not-use-rtti-or-exceptions)
    - [Prefer C++-Style Casts](#prefer-c-style-casts)
    - [Do Not Use Static Constructors](#do-not-use-static-constructors)
    - [Use of `class` and `struct` Keywords](#use-of-class-and-struct-keywords)
    ...
    - [Beware Unnecessary Copies with `auto`](#beware-unnecessary-copies-with-auto)
    - [Beware of Non-Determinism Due to Ordering of Pointers](#beware-of-non-determinism-due-to-ordering-of-pointers)
    - [Beware of Non-Deterministic Sorting Order of Equal Elements](#beware-of-non-deterministic-sorting-order-of-equal-elements)
- [Style Issues](#style-issues)
  - [The High-Level Issues](#the-high-level-issues)
    - [Self-Contained Headers](#self-contained-headers)
    - [Library Layering](#library-layering)
    - [`#include` as Little as Possible](#include-as-little-as-possible)
    - [Keep Internal Headers Private](#keep-internal-headers-private)
    - [Use Namespace Qualifiers to Define Previously Declared Symbols](#use-namespace-qualifiers-to-define-previously-declared-symbols)
    - [Use Early Exits and `continue` to Simplify Code](#use-early-exits-and-continue-to-simplify-code)
    - [Do Not Use `else` After `return`](#do-not-use-else-after-return)
  ...
  - [The Low-Level Issues](#the-low-level-issues)
    - [Name Types, Functions, Variables, and Enumerators Properly](#name-types-functions-variables-and-enumerators-properly)
    - [Assert Liberally](#assert-liberally)
    - [Do Not Use `using namespace std` or `using namespace llvm`](#do-not-use-using-namespace-std-or-using-namespace-llvm)
    - [Provide a Virtual Method Anchor for Classes in Headers](#provide-a-virtual-method-anchor-for-classes-in-headers)
    - [Do Not Use Default Labels in Fully Covered Switches Over Enumerations](#do-not-use-default-labels-in-fully-covered-switches-over-enumerations)
    - [Use Range-Based `for` Loops Wherever Possible](#use-range-based-for-loops-wherever-possible)
    - [Do Not Evaluate `end()` Every Time Through a Loop](#do-not-evaluate-end-every-time-through-a-loop)
    - [`#include <iostream>` Is Forbidden](#include-iostream-is-forbidden)
    - [Use `raw_ostream`](#use-raw_ostream)
    - [Avoid `std::endl`](#avoid-stdendl)
    - [Do Not Use `inline` When Defining a Function in a Class Definition](#do-not-use-inline-when-defining-a-function-in-a-class-definition)
  - [Microscopic Details](#microscopic-details)
    - [Spaces Before Parentheses](#spaces-before-parentheses)
    - [Prefer Preincrement](#prefer-preincrement)
    - [Namespace Indentation](#namespace-indentation)
    - [Restrict Visibility](#restrict-visibility)
    - [Do Not Use Braces on Simple Single-Statement Bodies of `if`/`else`/Loop Statements](#do-not-use-braces-on-simple-single-statement-bodies-of-ifelseloop-statements)
